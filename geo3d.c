...
void Polyhedron_cpt(
    ...
    for (int i = 0; i < (int)poly->num_faces; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            fe.pts[j+3] = Vector_lincom(1, fe.pts[j], maxd_exp, poly->face_list[i].n);
        }
        fe.sgn = 1;
    ...
    for (int i = 0; i < (int)poly->num_edges; i++) {
        ...
        if (angle <= PI/3) {
            if (ee.sgn == 1) {
                ee.pts[2] = Vector_lincom(1, ee.pts[0], maxd_exp, n_left);
                ee.pts[3] = Vector_lincom(1, ee.pts[1], maxd_exp, n_left);
                ee.pts[4] = Vector_lincom(1, ee.pts[0], maxd_exp, n_right);
                ee.pts[5] = Vector_lincom(1, ee.pts[1], maxd_exp, n_right);
            }
            else {
                ee.pts[2] = Vector_lincom(1, ee.pts[0], -maxd_exp, n_right);
                ee.pts[3] = Vector_lincom(1, ee.pts[1], -maxd_exp, n_right);
                ee.pts[4] = Vector_lincom(1, ee.pts[0], -maxd_exp, n_left);
                ee.pts[5] = Vector_lincom(1, ee.pts[1], -maxd_exp, n_left);
            }
            ...
            if (ee.sgn == 1) {
                ee.pts[2] = Vector_lincom(1, ee.pts[0], maxd_exp, n_left);
                ee.pts[3] = Vector_lincom(1, ee.pts[1], maxd_exp, n_left);
                ee.pts[4] = Vector_lincom(1, ee.pts[0], maxd_exp, n_mid);
                ee.pts[5] = Vector_lincom(1, ee.pts[1], maxd_exp, n_mid);
                EdgeExtrusion_cpt(ee, nx, ny, nz, x, y, z, f);
                ee.pts[2] = Vector_lincom(1, ee.pts[0], maxd_exp, n_mid);
                ee.pts[3] = Vector_lincom(1, ee.pts[1], maxd_exp, n_mid);
                ee.pts[4] = Vector_lincom(1, ee.pts[0], maxd_exp, n_right);
                ee.pts[5] = Vector_lincom(1, ee.pts[1], maxd_exp, n_right);
                EdgeExtrusion_cpt(ee, nx, ny, nz, x, y, z, f);
            }
            ...
            if (ee.sgn == 1) {
                ...
                ee.pts[2] = Vector_lincom(1, ee.pts[0], maxd_exp, n_left);
                ee.pts[3] = Vector_lincom(1, ee.pts[1], maxd_exp, n_left);
                ee.pts[4] = Vector_lincom(1, ee.pts[0], maxd_exp, n1);
                ee.pts[5] = Vector_lincom(1, ee.pts[1], maxd_exp, n1);
                EdgeExtrusion_cpt(ee, nx, ny, nz, x, y, z, f);
                ee.pts[2] = Vector_lincom(1, ee.pts[0], maxd_exp, n1);
                ee.pts[3] = Vector_lincom(1, ee.pts[1], maxd_exp, n1);
                ee.pts[4] = Vector_lincom(1, ee.pts[0], maxd_exp, n2);
                ee.pts[5] = Vector_lincom(1, ee.pts[1], maxd_exp, n2);
                EdgeExtrusion_cpt(ee, nx, ny, nz, x, y, z, f);
                ee.pts[2] = Vector_lincom(1, ee.pts[0], maxd_exp, n2);
                ee.pts[3] = Vector_lincom(1, ee.pts[1], maxd_exp, n2);
                ee.pts[4] = Vector_lincom(1, ee.pts[0], maxd_exp, n_right);
                ee.pts[5] = Vector_lincom(1, ee.pts[1], maxd_exp, n_right);
                EdgeExtrusion_cpt(ee, nx, ny, nz, x, y, z, f);
            }
